package com.tomasino.tracking
{
	...
	import com.tomasino.tracking.types.ITrackingType;
	...
	public class TrackingManager extends EventDispatcher
	{
		...
		private var _exitTimer:Timer = new Timer(500,1);
		...
		{
			// setting debug on the manager turns the debugs on for all tracking.
			_debug = value;
			if (_instance)
			{
				if (_instance._typeList)
				{
					for (var i:int = 0; i < _instance._typeList.length; ++i)
					{
						var trackingType:ITrackingType = _instance._typeList[i] as ITrackingType;
						trackingType.debug = _debug;
					}
				}
			}
