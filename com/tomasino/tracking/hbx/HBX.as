...
package com.tomasino.tracking.hbx
{
    import com.tomasino.net.Web;
    import com.tomasino.utils.Debug;
    import com.tomasino.utils.DynamicTimer;
	...
	public class HBX
	{
		...
		{ 
			...
			{
				Debug.out ("HBX::_initialize", "HBX Disabled due to Security Error");
				_availability = false;
			}
			Debug.out ("HBX::_initialize", "HBX Availability =", _availability);
			return true; 
		}
		...
		{
			if(_debug) Debug.out('HBX::TrackClick', sDescription);			
			if (_availability)
			{
		...
		{
			if(_debug) Debug.out('HBX::TrackRollover', sDescription + ' | ' + nDelay);			
			if (_availability)
			{
		...
		{
			if (!mlc) mlc = sMLC;
			if(_debug) Debug.out('HBX::TrackPageview', sMLC + ' | ' + sPageName);			
			if (_availability)
			{
		...
		{
			if(_debug) Debug.out('HBX::TrackExit', sDescription + ' | ' + sExitLink + ' | ' + sScope);
			if (_availability)
			{
		...
		private static function SendTrackingData(event:HBXEventDelayComplete):void
		{
			...
			if(_debug) Debug.out('HBX::SendTrackingData', event.TrackObject.EventType + ' | ' + sLinkName + ' | ' + sPageName);
		...
		private static function GetURL(e:TimerEvent):void
		{
			...
			if(_debug) Debug.out ('HBX::GetURL', sLink + ' | ' + sScope);
